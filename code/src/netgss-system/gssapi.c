...
static gss_buffer_t netgss_alloc_buffer(void) {
    return (gss_buffer_t) stat_alloc(sizeof(struct gss_buffer_desc));
}
...
static gss_OID netgss_alloc_oid(void) {
    return (gss_OID) stat_alloc(sizeof(struct gss_OID_desc));
}
...
CAMLprim value netgss_array_of_oid_set(value varg) {
    ...
    for (k=0; k<set->count; k++) {
        v2 = twrap_OID(1, netgss_copy_oid(set->elements[k]));
        Store_field(v1, k, v2);
    }
    CAMLreturn(v1);
}
